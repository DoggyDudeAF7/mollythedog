...
export function createSession(root,onFinish){let alive=true,paused=false,elapsed=0,last=performance.now(),request,cleanup;const listeners=[],timers=[],frames=[],keys=new Set();let hudSignature="";
 const ctx={root,keys,image,sound,get alive(){return alive;},get elapsed(){return elapsed;},get paused(){return paused;},
  hud(values){
   const signature=JSON.stringify(values);if(signature===hudSignature)return;hudSignature=signature;
   const hud=document.getElementById('game-hud'),entries=Object.entries(values);
   if(hud.children.length!==entries.length||entries.some(([label],i)=>hud.children[i]?.firstChild?.textContent!==label)){
    hud.replaceChildren(...entries.map(([label,value])=>{const d=document.createElement('div'),s=document.createElement('small'),b=document.createElement('strong');s.textContent=label;b.textContent=value;d.append(s,b);return d;}));
   }else entries.forEach(([,value],i)=>{const text=String(value);if(hud.children[i].lastChild.textContent!==text)hud.children[i].lastChild.textContent=text;});
  },
  status(message){const e=document.getElementById('game-status');if(e.textContent!==message)e.textContent=message;},
  on(target,event,fn,options){const guarded=e=>{
    ...
    fn(e);
   };target.addEventListener(event,guarded,options);listeners.push(()=>target.removeEventListener(event,guarded,options));return guarded;},
  ...
  pause(value){if(!alive)return;if(value&&!paused)root.dispatchEvent(new Event("arcade:pause"));paused=value;keys.clear();last=performance.now();root.inert=value;},
  dispose,
 ...
 function tick(now){if(!alive)return;const dt=Math.max(0,Math.min(.05,(now-last)/1000));last=now;if(!paused){elapsed+=dt;for(const timer of [...timers]){if(!alive)break;if(timer.at<=elapsed){if(timer.repeat)timer.at=elapsed+timer.repeat;else timers.splice(timers.indexOf(timer),1);timer.fn();}}for(const fn of [...frames]){if(!alive)break;fn(dt,elapsed);}}if(alive)request=requestAnimationFrame(tick);}
