...
let memory=blank(); let available=true; let dirty=false;
function read(){if(dirty)return memory;try{let raw;const text=localStorage.getItem(STORAGE_KEY);try{raw=JSON.parse(text||'null');}catch{return blank();}if(!raw||raw.version!==1)return blank();const clean=blank();clean.coins=number(raw.coins);clean.muted=raw.muted!==false;for(const {id} of catalog){const r=raw.results?.[id];if(r&&typeof r==='object')clean.results[id]={best:number(r.best),plays:number(r.plays),wins:number(r.wins)};}for(const id of Object.keys(raw.achievements||{})){if(badges.some(b=>b.id===id))clean.achievements[id]=true;}return clean;}catch{available=false;dirty=true;return memory;}}
export const badges=[
...
export function saveProgress(state){memory=structuredClone(state);dirty=true;try{localStorage.setItem(STORAGE_KEY,JSON.stringify(state));available=true;dirty=false;}catch{available=false;}window.dispatchEvent(new Event('ms:arcade-changed'));}
