(function () {
  ...
  function isExcluded(node, boundary) {
    ...
    while (parent) {
      if (
        parent.nodeType === 1 &&
        (excludedTags[parent.tagName] || parent.isContentEditable)
      ) {
        return true;
      }
  ...
  function replaceShortcodesInTextNode(node) {
    ...
    while ((match = dogTextPattern.exec(text)) !== null) {
      ...
      fragment.appendChild(
        createDogEmoji(name || nameFromContext(node, match[2]))
      );
  ...
  function renderDogEmojis(root) {
    ...
    if (root.nodeType === 3) {
      ...
      if (
        !isExcluded(root, root) &&
        dogTextPattern.test(root.nodeValue)
      ) {
        nodes.push(root);
      }
    } else {
      ...
      while ((current = walker.nextNode())) {
        ...
        if (
          !isExcluded(current, root) &&
          dogTextPattern.test(current.nodeValue)
        ) {
          nodes.push(current);
        }
      }
    ...
    for (i = 0; i < nodes.length; i += 1) {
      replaceShortcodesInTextNode(nodes[i]);
    }
  }
...
function initMollyShainaAnimations() {
  ...
  if (window.MutationObserver && document.body) {
    ...
    dogEmojiObserver.observe(document.body, {
      childList: true,
      characterData: true,
      subtree: true
    });
    ...
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          document.title.includes("FAQ") ||
          document.title.includes("Traits")
        ) {
  ...
  if (
    habitIds.every(id => document.getElementById(id))
  ) {
    let data = {
      naps: 3,
      windows: 4,
    ...
    const habitStorageKey = location.pathname.includes("poppy")
      ? "poppyHabits"
      : "mollyHabits";

    try {
      data =
        JSON.parse(localStorage.getItem(habitStorageKey)) ||
        data;
    } catch {}
  ...
  if (questionInput && askBtn && answer) {
    ...
    function getDogResponse(question) {
      ...
      if (
        ...
        return "If it involves food, I approve immediately. Especially cheese. Cheese is truth.";
      }

      if (includesAny(compact, ["rain"])) {
        return "Rain is suspicious, loud, and wet. I recommend avoiding it forever if possible.";
      }

      if (includesAny(compact, ["walk"])) {
        return "Walks are acceptable. Especially when I am pretending I do not want them.";
      }

      if (includesAny(compact, ["toy"])) {
        return "Toys are decorative emotional objects. I inspect them, then abandon them dramatically.";
      }

      if (includesAny(compact, ["who are you"])) {
        return "I am Molly. I am responsible for monitoring windows, naps, and snack security.";
      }

      if (includesAny(compact, ["help"])) {
        return "I can help emotionally, but I may get distracted by a distant sound mid-sentence.";
      }

      if (includesAny(compact, ["love"])) {
        return "Love is acceptable. Especially when it comes with snacks or soft blankets.";
      }

      if (includesAny(compact, ["sad"])) {
        return "That sounds emotionally complicated. I recommend lying down until it passes.";
      }

      if (includesAny(compact, ["angry"])) {
        return "Anger is too high-energy. Please convert it into a nap immediately.";
      }

      if (includesAny(compact, ["school"])) {
        return "School sounds like a long indoor walk without snacks. I am unsure about it.";
      }

      if (includesAny(compact, ["home"])) {
        return "Home is the safest place for thinking, sleeping, and snack negotiations.";
      }

      if (includesAny(compact, ["night"])) {
    ...
    askBtn.addEventListener(
      ...
      async () => {
        ...
        if (!q) {
          typeText(
            `${dogName} is waiting for a question...`
          );
...
// This matters when animations.js is loaded dynamically
// or with a loader after DOMContentLoaded has already fired.
