(function () {
  ...
  function nameFromContext(node, symbol) {
    ...
    var named =
      text.match(/\b(molly|poppy|shaina)\b/i);
    ...
    if (/poppy/i.test(context)) {
      return "poppy";
    }

    if (/shaina/i.test(context)) {
      return "shaina";
    }

    if (/molly/i.test(context)) {
      return "molly";
    }
    ...
    if (
      /\/poppy(?:-|\/)/i.test(location.pathname)
    ) {
      return "poppy";
    }

    if (
      /\/shaina(?:-|\/)/i.test(location.pathname)
    ) {
      return "shaina";
    }
  ...
  function replaceShortcodesInTextNode(node) {
    ...
    dogTextPattern.lastIndex =
      0;

    while (
      (match = dogTextPattern.exec(text)) !== null
    ) {
      if (
